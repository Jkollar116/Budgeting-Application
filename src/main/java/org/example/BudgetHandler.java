...
public class BudgetHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        System.out.println("BudgetHandler invoked: " + exchange.getRequestMethod());
    ...
    private void handleGet(HttpExchange exchange, String idToken, String localId) throws IOException {
        ...
        System.out.println("Firestore GET response code (budgets): " + code);
        if (code == 200) {
    ...
    private void handlePost(HttpExchange exchange, String idToken, String localId) throws IOException {
        ...
        String month = getJsonValue(body, "month");
        String amountStr = getJsonValue(body, "amount");
        double amount;
        try {
        ...
        if (month == null || !month.matches("\\d{4}-\\d{2}")) {
            System.out.println("Invalid month format received: " + month);
            exchange.sendResponseHeaders(400, -1);
            return;
        }
        String json = "{\"fields\":{"
        ...
        int code = conn.getResponseCode();
        System.out.println("Firestore POST response code (budgets): " + code);
        if (code == 200 || code == 201) {
            byte[] msg = "Budget added successfully.".getBytes(StandardCharsets.UTF_8);
    ...
    private void handleDelete(HttpExchange exchange, String idToken, String localId) throws IOException {
        ...
        if (docId == null || docId.isEmpty()) {
            System.out.println("DELETE missing docId parameter (budgets)");
            exchange.sendResponseHeaders(400, -1);
            return;
        }
        String urlStr = "https://firestore.googleapis.com/v1/projects/cashclimb-d162c/databases/(default)/documents/Users/"
                + localId + "/Budgets/" + docId;
        System.out.println("Firestore DELETE URL (budgets): " + urlStr);
        HttpURLConnection conn = openFirestoreConnection(urlStr, "DELETE", idToken);
        int code = conn.getResponseCode();
        System.out.println("Firestore DELETE response code (budgets): " + code);
    ...
    private void logError(HttpURLConnection conn) throws IOException {
        ...
        if (err != null) {
            System.out.println("Firestore error (budgets): " + readAll(err));
        }
    }
